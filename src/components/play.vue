...
<template>
  <div>
    <div style="position: absolute;top: 20px;left: 100px; ">
      ...
      <div style="margin-top:50px; border:dotted">
        <oi>
          ...
          <li>按空白鍵抓取物品</li>
          <br><br><br>
          <div><el-button @click="start_game" v-show="play_lock">開始遊戲</el-button></div>
          <br><br>
        </oi>
      </div>
    </div>
    ...
    <div style="position: absolute;top: 120px;right: 100px;">
      <el-button  style="font-size: 30px;" icon="el-icon-s-cooperation" type="primary" @click="open_prize_form"></el-button>
      <el-dialog title="填寫獎品運送資訊" :visible.sync="prize_form">
      <div style="display: flex;">
        <div style="width:50%">
          <el-form :model="form" label-width="80px">
            <el-form-item prop="name" label="姓名" >
              <el-input v-model="form.name" style="width:200px; display: block;" placeholder="請輸入姓名"></el-input>
            </el-form-item>
            <el-form-item prop="cellphone" label="手機" >
              <el-input v-model.number="form.cellphone" style="width:200px; display: block;" placeholder="請輸入手機"></el-input>
            </el-form-item>
            <el-form-item prop="adddress" label="地址" >
              <el-input v-model="form.adddress" style="width:300px; display: block;" placeholder="請輸入地址"></el-input>
            </el-form-item>
          </el-form>
        </div>

        <div style="width:50%">
          <div v-for="(item, index) in product_url" style="float: left; margin-top: 20px;">
            <div>
              <img  :src="item" style="width:150px; height:150px;">
            </div>
            <div>
              <el-checkbox v-model="prize_selection[index]" label="" >{{index}}</el-checkbox>
            </div>
          </div>
        </div>
      </div>
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="danger"  @click="prize_form=false" plain>取消</el-button>
      </div>
      </el-dialog>
    </div>
...
export default {
  ...
  created(){
    ...
    this.axios.post("/get_user_prize",{
      user : this.user
    }).then((response)=>{
      if(response.data != "")
        this.prize_info = JSON.parse(response.data);
      else
        this.prize_info = [];
  ...
  methods:{
    ...
    del_prize(){
      this.axios.post('/del_user_prize', {
        user:  this.user,
    ...
    initThree() {
        ...
        var new_play_lock = true, old_play_lock=true;
        var bingo = undefined;
        ...
        const bgTexture5 = new THREE.TextureLoader().load( require('@/assets/ground.jpg') );
        const bgmaterial5 = new THREE.MeshBasicMaterial( {map: bgTexture5, side: THREE.DoubleSide} );
        ...
        const bgTexture6 = new THREE.TextureLoader().load( require('@/assets/ceiling.jpg') );
        const bgmaterial6 = new THREE.MeshBasicMaterial( {map: bgTexture6, side: THREE.DoubleSide} );
        ...
        scene.add( bgplane6 );
				...
				var material = new THREE.MeshPhongMaterial( {transparent: true, opacity: 0.4, side: THREE.DoubleSide} );
				...
				/*箱子*/
				var texture = new THREE.TextureLoader().load( require('@/assets/crate.gif') );
        ...
        const coinholeTexture = new THREE.TextureLoader().load( require('@/assets/coin_hole2.png') );
        const coinholeMaterial = new THREE.MeshBasicMaterial( {map: coinholeTexture, side: THREE.DoubleSide} );
        ...
        const coinTexture = new THREE.TextureLoader().load( require('@/assets/coin.png') );
        const coinMaterial = new THREE.MeshBasicMaterial( {map: coinTexture, side: THREE.DoubleSide} );
        ...
        var joystickHeadgeometry = new THREE.SphereGeometry(10, 32, 32);
				var joystickHeadTexture = new THREE.TextureLoader().load( require('@/assets/UV_Grid_Sm.jpg') );
				var joystickHeadMaterial = new THREE.MeshPhongMaterial({map: joystickHeadTexture});
        ...
        var joystickBodyTexture = new THREE.TextureLoader().load( require('@/assets/UV_Grid_Sm.jpg') );
				var joystickBodymaterial = new THREE.MeshBasicMaterial( {map: joystickBodyTexture} );
        ...
        var clawTexture = new THREE.TextureLoader().load( require('@/assets/UV_Grid_Sm.jpg') );
        ...
        var controlGeometry = new THREE.SphereGeometry(50, 32, 32);
				var controlTexture = new THREE.TextureLoader().load( require('@/assets/UV_Grid_Sm.jpg') );
        ...
        var lineTexture = new THREE.TextureLoader().load( require('@/assets/UV_Grid_Sm.jpg') );
        ...
        let rwallBody = new CANNON.Body({mass: 0,shape: groundShape,position: new THREE.Vector3( 150, 0, 0 ),material: rwallCM})
        rwallBody.quaternion.setFromAxisAngle(new CANNON.Vec3(0, 1, 0), -Math.PI / 2 )
        world.add(rwallBody)
        ...
        let lwallBody = new CANNON.Body({mass: 0,shape: groundShape,position: new CANNON.Vec3( -150, 0, 0 ),material: lwallCM})
        lwallBody.quaternion.setFromAxisAngle(new CANNON.Vec3(0, 1, 0), Math.PI / 2 )
        world.add(lwallBody)

        //牆壁(前)
        let fwallCM = new CANNON.Material()
        let fwallBody = new CANNON.Body({mass: 0,shape: groundShape,position: new THREE.Vector3( 0, 0, 150 ),material: fwallCM})
        ...
        let bwallBody = new CANNON.Body({mass: 0,shape: groundShape,position: new THREE.Vector3( 0, 0, -150 ),material: bwallCM})
        world.add(bwallBody)
        ...
        var Bafflematerial = new THREE.MeshPhongMaterial( {color: 0xFF11FF ,transparent: true, opacity: 0.4, side: THREE.DoubleSide} );
        ...
        for(let i=0; i<product_amount; i++){
          ...
          // 物品mesh
          mesh_array[i] = new THREE.Mesh(new THREE.BoxGeometry(44, 44, 44), new THREE.MeshStandardMaterial({ map: bodyTexture }))
        ...
        var controls = new TControls.TrackballControls( camera, renderer.domElement );
        ...
        function animate() {
            ...
            controls.update();
            mesh_array.forEach((item,index)=>{ mesh_array[index].position.copy(body_array[index].position) })
            ...
            if(up && !clawlock){
              ...
              if(joystickSystem.rotation.x > -1)
                joystickSystem.rotation.x -= 0.1;
            }
            if(down && !clawlock){
              clawSystem.position.z += 2;
              if(joystickSystem.rotation.x < 1)	
            ...
            if(left && !clawlock){
              ...
              if(joystickSystem.rotation.z < 1)	
                joystickSystem.rotation.z += 0.1;
            }
            if(right && !clawlock){
              clawSystem.position.x += 2;
              if(joystickSystem.rotation.z > -1)	
            ...
            if(!up && !down && !left && !right){
              if(joystickSystem.rotation.x < 0)
                joystickSystem.rotation.x += 0.1;
              if(joystickSystem.rotation.x > 0)
                joystickSystem.rotation.x -= 0.1;
              if(joystickSystem.rotation.z < 0)
                joystickSystem.rotation.z += 0.1;
              if(joystickSystem.rotation.z > 0)
                joystickSystem.rotation.z -= 0.1;
            }
              ...
              if(close){
                ...
                if(clawMesh2.rotation.z >2){
                  body_array.forEach((i) => {
                    if (i.position.x < clawSystem.position.x+15 && i.position.x > clawSystem.position.x-15 && 
                      i.position.z < clawSystem.position.z+15 && i.position.z > clawSystem.position.z-15)
                        bingo = i;
                  });
                  ...
                  setTimeout(function(){
                    ...
                    body_array.forEach((item, index)=>{
                        if(item.position.y < -10){
                          ...
                          world.remove(item);
                          that.axios.post("/add_user_prize",{
                            user : that.user,
                            prize_info: JSON.parse(sessionStorage["play_info"])['id'],
                          }).then((response)=>{
                                that.$message({message:"恭喜獲得獎品", type:"success"});
                          })
                        }
                    })
